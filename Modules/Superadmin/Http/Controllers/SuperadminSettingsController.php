...
class SuperadminSettingsController extends BaseController
{
    ...
    public function edit()
    {
        ...
        $default_values = [
            ...
            'FATOORAH_BASE_URL' => $is_demo ? null : env('FATOORAH_BASE_URL'),
            'FATOORAH_TOKEN' => $is_demo ? null : env('FATOORAH_TOKEN'),
        ...
        return view('superadmin::superadmin_settings.edit')->with(compact(
            'currencies', 'settings', 'superadmin_version',
            'mail_drivers', 'languages', 'default_values',
            'backup_disk', 'cron_job_command'
        ));
    }
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $system_settings = $request->only([
                ...
                'welcome_email_subject', 'welcome_email_body', 'additional_js',
                'additional_css', 'offline_payment_details'
            ]);

            $checkboxes = [
                'enable_business_based_username', 'superadmin_enable_register_tc',
            ...
            foreach ($checkboxes as $checkbox) {
                $system_settings[$checkbox] = $request->has($checkbox) ? 1 : 0;
            }
            ...
            $env_settings = $request->only([
                'APP_NAME', 'APP_TITLE', 'APP_LOCALE', 'MAIL_DRIVER', 'MAIL_HOST', 'MAIL_PORT',
                'MAIL_USERNAME', 'MAIL_PASSWORD', 'MAIL_ENCRYPTION', 'MAIL_FROM_ADDRESS',
                'MAIL_FROM_NAME', 'STRIPE_PUB_KEY', 'STRIPE_SECRET_KEY', 'PAYPAL_MODE',
                'PAYPAL_SANDBOX_API_USERNAME', 'PAYPAL_SANDBOX_API_PASSWORD',
                'PAYPAL_SANDBOX_API_SECRET', 'PAYPAL_LIVE_API_USERNAME',
                'PAYPAL_LIVE_API_PASSWORD', 'PAYPAL_LIVE_API_SECRET', 'BACKUP_DISK',
                'DROPBOX_ACCESS_TOKEN', 'RAZORPAY_KEY_ID', 'RAZORPAY_KEY_SECRET',
                'FATOORAH_BASE_URL', 'FATOORAH_TOKEN', 'PESAPAL_CONSUMER_KEY',
                'PESAPAL_CONSUMER_SECRET', 'PESAPAL_LIVE', 'PUSHER_APP_ID', 'PUSHER_APP_KEY',
                'PUSHER_APP_SECRET', 'PUSHER_APP_CLUSTER', 'GOOGLE_MAP_API_KEY',
                'PAYSTACK_SECRET_KEY', 'PAYSTACK_PUBLIC_KEY', 'FLUTTERWAVE_PUBLIC_KEY',
                'FLUTTERWAVE_SECRET_KEY', 'FLUTTERWAVE_ENCRYPTION_KEY', 'MAPBOX_ACCESS_TOKEN'
            ]);

            $env_settings['ALLOW_REGISTRATION'] = $request->has('ALLOW_REGISTRATION') ? 'true' : 'false';
            ...
            if (is_writable($env_path) && file_put_contents($env_path, $env_content)) {
                ...
                $output = ['success' => 0, 'msg' => 'Could not write to .env file. Check permissions.'];
            }
        } catch (\Exception $e) {
        ...
        return redirect()->action('\Modules\Superadmin\Http\Controllers\SuperadminSettingsController@edit')
            ->with('status', $output);
    }
}
