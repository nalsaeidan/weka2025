...
class SubscriptionController extends BaseController
{
    ...
    public function __construct(ModuleUtil $moduleUtil = null,FatoorahServices $fatoorahServices)
    {
	...
	public function subscription(Request $request,$package_id){
    	$user=auth()->user();
       $package = Package::active()->find($package_id);
          if($package->price >0){
             $requestData= [
            ...
            "CallBackUrl"=>route('subscripe.package.confirm'),
		...
		public function confirmPayment(Request $request){
         ...
         $package = Package::active()->find((int)$responseData->Data->CustomerReference);
            ...
            $gateway=$responseData->Data->InvoiceTransactions[0]->PaymentGateway??"VISA/MASTER";
            $this->_add_subscription($business_id, $package->id, $gateway, $payment_transaction_id, $user->id);
            // DB::commit();
