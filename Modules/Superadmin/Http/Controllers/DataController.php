...
class DataController extends Controller
{
     ...
     * @return null
     */
    public function modifyAdminMenu($sub)
    {
        if (auth()->user()->can('superadmin')) {
            // Menu::modify(
            //     'admin-sidebar-menu',
                // function ($sub) {
                    $sub->url(action('\Modules\Superadmin\Http\Controllers\SuperadminController@index'), __('superadmin::lang.superadmin'), ['icon' => 'fa fas fa-users-cog', 'active' => request()->segment(1) == 'superadmin'])->order(111);
                // }
            // );       
        }
        ...
        if (auth()->user()->can('superadmin.access_package_subscriptions') && auth()->user()->can('business_settings.access')) {
            ...
            Menu::modify(
                'admin-sidebar-menu',
                function ($menu) {
                    $menu->url(action('\Modules\Superadmin\Http\Controllers\SubscriptionController@index'), 'باقاتي', ['icon' => 'fa fas fa-sync', 'active' => request()->segment(1) == 'subscription'])->order(888);
                }
            ); 
        }
    }
