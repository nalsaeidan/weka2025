                                ...
                                <ul class="dropdown-menu">
                                    ...
                                    <!-- The user image in the menu -->
                                                @if (auth()->user()->can('product.view'))
                                                    <a href="{{route('labels.show')}}">
                                                        <li><i class="fal fa-angle-double-left"></i>{{__('barcode.print_labels')}}</li>
                                                    </a>
                                                @endif
                                                @if (auth()->user()->can('product.create'))
                                                    <a href="{{route('variation-templates.index')}}">
                                                        <li><i class="fal fa-angle-double-left"></i>{{__('product.variations')}}</li>
                                                    </a>
                                                @endif
                                                @if (auth()->user()->can('product.create'))
                                                    <a href="{{route('import-products-index')}}">
                                                        <li><i class="fal fa-angle-double-left"></i>{{__('product.import_products')}}</li>
                                                    </a>
                                                @endif
                                                @if (auth()->user()->can('product.opening_stock')) 
                                                    <a href="{{route('import-opening-stock-index')}}">
                                                        <li><i class="fal fa-angle-double-left"></i>{{__('lang_v1.import_opening_stock')}}</li>
                                                    </a>
                                                @endif
                                                @if (auth()->user()->can('product.create'))
                                                    <a href="{{route('selling-price-group.index')}}">
                                                        <li><i class="fal fa-angle-double-left"></i>{{__('lang_v1.selling_price_group')}}</li>
                                                    </a>
                                                @endif
                                                @if (auth()->user()->can('unit.view') || auth()->user()->can('unit.create'))
                                                    <a href="{{route('units.index')}}">
                                                    <li><i class="fal fa-angle-double-left"></i>{{__('unit.units')}}</li>
                                                </a>
                                                @endif
                                                @if (auth()->user()->can('category.view') || auth()->user()->can('category.create'))
                                                    <a href="{{route('taxonomies.index'). '?type=product'}} ">
                                                    <li><i class="fal fa-angle-double-left"></i>{{__('category.categories')}}</li>
                                                </a>
                                                @endif
                                                @if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create'))
                                                    <a href="{{route('brands.index')}}">
                                                    <li><i class="fal fa-angle-double-left"></i>{{__('brand.brands')}}</li>
                                                </a>
                                                @endif
                                                @if (auth()->user()->can('brand.view') || auth()->user()->can('brand.create'))
                                                    <a href="{{route('warranties.index')}}">
                                                    <li><i class="fal fa-angle-double-left"></i>{{__('lang_v1.warranties')}}</li>
                                                </a>
                                                @endif
                                </ul>
                            </li>
...
@section('javascript')
    ...
    return '<table cellpadding="5" cellspacing="0" border="0" style="padding-left:50px;">'+
        '<tr>'+
            '<td>{{ $custom_labels['product']['custom_field_1'] ?? __('lang_v1.product_custom_field1') }}: </td>'+
            '<td>'+d.product_custom_field1+'</td>'+
        '</tr>'+
        '<tr>'+
            '<td>{{ $custom_labels['product']['custom_field_2'] ?? __('lang_v1.product_custom_field2') }}</td>'+
            '<td>'+d.product_custom_field2+'</td>'+
        '</tr>'+
        '<tr>'+
            '<td>{{ $custom_labels['product']['custom_field_3'] ?? __('lang_v1.product_custom_field3') }}</td>'+
            '<td>'+d.product_custom_field3+'</td>'+
        '</tr>'+
            '<tr>'+
            '<td>{{ $custom_labels['product']['custom_field_4'] ?? __('lang_v1.product_custom_field4') }}</td>'+
            '<td>'+d.product_custom_field4+'</td>'+
        '</tr>'+
    '</table>';
}
        $(document).ready( function(){
            product_table = $('#product_table').DataTable({
               ...
               // scrollCollapse: true,
                ...
                columns: [
                		...
                		{
                			"className":      'details-control text-success cursor-pointer no-print',
                			"orderable":      false,
                			"data":           null,
                			"defaultContent": '',
            			},
    		...
    		$('table#product_table>tbody').on('click', 'td.details-control', function () {
