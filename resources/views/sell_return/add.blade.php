					...
					<table class="table bg-gray" id="sell_return_table">
				        ...
				        <tbody>
				          	@foreach($sell->sell_lines as $sell_line)
				          		@php
					                $check_decimal = 'false';
					                if($sell_line->product->unit && $sell_line->product->unit->allow_decimal == 0){
					                    $check_decimal = 'true';
					                }
									if($sell_line->product->unit){
					                	$unit_name = $sell_line->product->unit->short_name;
                                    }
					                if($sell_line->sub_unit && $sell_line->sub_unit != null) {
				            ...
				            <tr>
				              	...
				              	<td>
									...
									{{ $sell_line->product->name }}
									 @if( $sell_line->variations->product_variation->name != "DUMMY")
									  - {{ $sell_line->variations->product_variation->name}}
									  @endif
									  @if($sell_line->variations->name != 'DUMMY')
