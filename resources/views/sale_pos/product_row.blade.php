...
<tr class="product_row" data-row_index="{{$row_count}}" @if(!empty($so_line)) data-so_id="{{$so_line->transaction_id}}" @endif>
	...
	<td>
		<input disabled type="text" name="products[{{$row_count}}][purchase_price]" class="form-control pos_purchase_price input_number mousetrap" value="{{@num_format($dpp_inc_tax)}}">
	</td>
	{{-- المخزون الحالي	 --}}
	<td>
		<span data-is_quantity="true" class="display_currency"data-currency_symbol=false >{{$product->current_stock ?? 0}}</span>{{$product->unit}}
