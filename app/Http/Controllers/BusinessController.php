...
class BusinessController extends Controller
{
   ...
   private $fatoorahServices;
    public function __construct(BusinessUtil $businessUtil, RestaurantUtil $restaurantUtil, ModuleUtil $moduleUtil,FatoorahServices $fatoorahServices)
    {
  ...
  public function postRegister(Request $request)
    {
        ...
        try {
            ...
            //Create owner.
            $owner_details = $request->only(['surname', 'first_name', 'last_name', 'username', 'email', 'password', 'language','contact_number']);
            ...
            "InvoiceTax"=>  $package->tax_p,
            "CustomerEmail"=>$user->email,
            "CallBackUrl"=>route('fatoorah.success.payment'),
            "ErrorUrl"=>route('fatoorah.error.payment'),
            "Language"=>"ar",
          ...
          $response= $this->fatoorahServices->sendPayment($requestData);
	 ...
	 public function successPayment(Request $request){
        	...
        	$responseData= $this->fatoorahServices->getPaymentStatus($data);
  
         $package = Package::active()->find((int)$responseData->Data->CustomerReference);
     		$user=auth()->user();
     		if($user){
            
             $dates = $this->_get_package_dates($user->business_id, $package);
             $subscription = ['business_id' => $user->business_id,
                        'package_id' => $package->id,
                        'paid_via' => $responseData->Data->InvoiceTransactions[0]->PaymentGateway??"VISA/MASTER",
                        'payment_transaction_id' => @$responseData->Data->InvoiceTransactions[0]->TransactionId,
     ...
     // if((int)$responseData->Data->CustomerReference){
     // $package_id=(int)$responseData->Data->CustomerReference;
     //    return redirect()->route('register-pay', ['package_id' => $package_id]);
     // }
//      $user=auth()->user();
//      // return $responseData->Data->InvoiceTransactions[0]->PaymentGateway;
//      $this->_add_subscription($user->business_id,(int)$responseData->Data->CustomerReference,$responseData->Data->InvoiceTransactions[0]->PaymentGateway,$responseData->Data->InvoiceTransactions[0]->TransactionId,$user->id);
...
//     }
    ...
    public function errorPayment(Request $request){
        ...
        $data['Key']=$request->paymentId;
        $data['KeyType']="paymentId";
       return  $responseData= $this->fatoorahServices->getPaymentStatus($data);
        return $responseData['Data']['InvoiceId'];
    }
    ...
    public function postBusinessSettings(Request $request)
    {
        ...
        try {
            ...
            $business_details = $request->only(['name', 'start_date', 'currency_id', 'tax_label_1', 'tax_number_1', 'tax_label_2', 'tax_number_2', 'default_profit_percent', 'default_sales_tax', 'default_sales_discount', 'sell_price_tax', 'sku_prefix', 'time_zone', 'fy_start_month', 'accounting_method', 'transaction_edit_days', 'sales_cmsn_agnt', 'item_addition_method', 'currency_symbol_placement', 'on_product_expiry',
                'stop_selling_before', 'default_unit', 'expiry_type',
 ...
 $business_details['date_format'] = 'Y-m-d';
    ...
    return view('layouts.partials.pricing');
    }
}
