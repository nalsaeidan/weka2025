...
class ReportController extends Controller
{
    ...
    public function getRegisterReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($start_date) && !empty($end_date)) {
                $registers->where('cash_registers.created_at', '>=', $start_date)
                        ->where('cash_registers.created_at', '<=', $end_date);
            }
            return Datatables::of($registers)
                ...
                ->addColumn('action', '<button type="button" data-href="{{action(\'CashRegisterController@show\', [$id])}}" class="btn btn-xs btn-info btn-modal" 
                    data-container=".view_register"><i class="fas fa-eye" aria-hidden="true" title=" @lang("messages.view")"></i></button> @if($status != "close" && auth()->user()->can("close_cash_register"))<button type="button" data-href="{{action(\'CashRegisterController@getCloseRegister\', [$id])}}" class="btn btn-xs btn-danger btn-modal" 
                        data-container=".view_register"><i class="fas fa-window-close" title="@lang("messages.close")"></i></button> @endif')
    ...
    public function purchasePaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            if (!empty($start_date) && !empty($end_date)) {
                ...
                $query->where('paid_on', '>=', $start_date)
                ->where('paid_on', '<=', $end_date);
            }
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', '<button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action("TransactionPaymentController@viewPayment", [$DT_RowId]) }}" title="@lang("messages.view")"><i class="fas fa-eye" aria-hidden="true"></i>
                    </button> @if(!empty($document))<a href="{{asset("/uploads/documents/" . $document)}}" class="btn btn-success btn-xs" download="" title="@lang("purchase.download_document")"><i class="fa fa-download"></i> </a>@endif')
    ...
    public function sellPaymentReport(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->addColumn('action', '<button type="button" class="btn btn-primary btn-xs view_payment" data-href="{{ action("TransactionPaymentController@viewPayment", [$DT_RowId]) }}" title="@lang("messages.view")"><i class="fas fa-check-square"></i>
                    </button> @if(!empty($document))<a href="{{asset("/uploads/documents/" . $document)}}" class="btn btn-success btn-xs" download="" title=" @lang("purchase.download_document")"><i class="fa fa-download"></i></a>@endif')
        ...
        if (request()->ajax()) {
            ...
            return Datatables::of($query)
                ...
                ->editColumn('total_purchase_price', function ($row) {
                    $subpurchasetotal = $row->purchase_price * $row->quantity;

                    return '<span class="display_currency total_purchase_price" data-currency_symbol=true data-orig-value="' . $subpurchasetotal . '">' . $subpurchasetotal . '</span>';
                })
                ...
                ->rawColumns(['total_purchase_price','subtotal', 'selling_price', 'quantity', 'purchase_price', 'sale_invoice_no', 'purchase_ref_no', 'supplier', 'customer'])
                ->make(true);
        }
        ...
        if (request()->ajax()) {
            ...
            if (!empty(request()->start_date) && !empty(request()->end_date)) {
                ...
                $activities->where('activity_log.created_at', '>=', $start)
                            ->where('activity_log.created_at', '<=', $end);
            }
