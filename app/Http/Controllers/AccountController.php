...
class AccountController extends Controller
{
    ...
    public function index()
    {
        ...
        // $xnul = Account::where('business_id',null)->get();
        // // dd($xnul);
        // foreach($xnul as $x){
        ...
        if (request()->ajax()) {
            ...
            ->leftJoin('users AS u', 'accounts.created_by', '=', 'u.id')
                                ->Where('accounts.business_id',$business_id)->orWhere('accounts.business_id',null)
            ...
            return DataTables::of($accounts)
                            ...
                            ->addColumn('action', function ($row ) {
                                $html = '<button data-href="'. action('AccountController@edit',[$row->id]).'" data-container=".account_model" class="btn btn-xs btn-primary btn-modal"title="تعديل"><i class="glyphicon glyphicon-edit"></i></button>&nbsp;';
                               
                                if ($row->id == 1) {
                                    $html .= '<a href="' . action('ContactController@index', ['type' => 'customer']) .'" class="btn btn-warning btn-xs" title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }
                                elseif ($row->id ==2) {
                                    $html .= '<a href="' . action('ContactController@index', ['type' => 'supplier']) .'" class="btn btn-warning btn-xs" title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }
                                elseif ($row->id == 3) {
                                    $html .= '<a href="' . action('ReportController@getStockReport') .'" class="btn btn-warning btn-xs" title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }
                                elseif ($row->id == 4) {
                                    $html .= '<a href="' . action('ReportController@getProfitLoss') .'" class="btn btn-warning btn-xs" title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }
                           	 	elseif ($row->id == 5) {
                                    $html .= '<a href="' . action('AccountReportsController@balanceSheet1') .'" class="btn btn-warning btn-xs" title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }
                            	elseif ($row->id == 100) {
                                    $html .= '<a href="' . action('ReportController@getTaxReport') .'" class="btn btn-warning btn-xs" title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }
                                elseif ($row->id == 200) {
                                    $html .= '<a href="' . action('ReportController@getTaxReport') .'" class="btn btn-warning btn-xs" title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }
                                elseif ($row->id == 300) {
                                    ...
                                    $html .= '<a href="' . action('AccountController@show', [$row->id]) .'" class="btn btn-warning btn-xs " title="'.__("account.account_book").'"> <i class="fa fa-book"></i> </a>&nbsp;';
                                }

                                if($row->is_closed == 0){
                                    $html .= '<button data-href="'.action('AccountController@getFundTransfer',[$row->id]).'" class="btn btn-xs btn-info btn-modal" data-container=".view_modal" title="'.__("account.fund_transfer").'"><i class="fas fa-exchange-alt"></i> </button>

                                <button data-href="'. action('AccountController@getDeposit',[$row->id]).'" class="btn btn-xs btn-success btn-modal" data-container=".view_modal" title="'.__("account.deposit").'"><i class="fas fa-money-bill-alt"></i> </button>

                                <button data-url="'.action('AccountController@close',[$row->id]).'" class="btn btn-xs btn-danger close_account" title="' .__("messages.close").'"><i class="fa fa-power-off"></i></button>';
                                }
                                if($row->is_closed == 1){
                                    $html .= '<button data-url="'.action('AccountController@activate',[$row->id]).'" class="btn btn-xs btn-success activate_account" title="'.__("messages.activate").'"><i class="fa fa-power-off"></i></button>';
                                }
        ...
        $account_types = AccountType::whereNull('parent_account_type_id')
                                     ->with(['sub_types'])
                                     ->get();
        ...
        if (request()->ajax()) {
            ...
            ->leftJoin('contacts AS c', 'tp.payment_for', '=', 'c.id')
                            ...
                            ->select(['account_transactions.type', 'account_transactions.amount', 'operation_date',
                                'sub_type', 'transfer_transaction_id','restrict_id',
...
//             ->leftJoin('users AS u', 'account_transactions.created_by', '=', 'u.id')
//             ->leftJoin('contacts AS c', 'tp.payment_for', '=', 'c.id')
//                             ->where('A.business_id', $business_id)->orWhere('A.business_id', null)
//                             ->where('A.id', $id)
//                             ->with(['transaction', 'transaction.contact', 'transfer_transaction', 'media', 'transfer_transaction.media'])
//                             ->select(['account_transactions.type', 'account_transactions.amount', 'operation_date',
//                                 'sub_type', 'transfer_transaction_id','restrict_id',
...
//                             ->make(true);
//         }
//         $account = Account::where('business_id', $business_id)->orWhere('business_id', null)
//                         ->with(['account_type', 'account_type.parent_account'])
//                         ->findOrFail($id);
...
//     }
    ...
    public function __getPaymentDetails($row)
    {
        ...
        if (!empty($row->sub_type)) {
            ...
            if($row->sub_type == 'restrict'){
                if ($row->type == 'credit') {
                    $details ='( ' . __('account.' . $row->sub_type) .' ) :' .' يحمل رقم '.'(' .$row->restrict_id . ')';
                }
                else{
                    $details ='( ' . __('account.' . $row->sub_type) .' ) :' .' يحمل رقم '.'(' .$row->restrict_id .')';
                }
            }
            elseif (in_array($row->sub_type, ['fund_transfer', 'deposit']) && !empty($row->transfer_transaction)) {
                if ($row->type == 'credit') {
