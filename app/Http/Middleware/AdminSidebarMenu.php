...
class AdminSidebarMenu
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        Menu::create('admin-sidebar-menu', function ($menu) {
            ...
            //Accounts menu
            $menu->url(action('AccountController@index'), __('accounts.account_menu'), ['icon' => 'fa fas fa-tachometer-alt', 'active' => request()->segment(10) == 'accounts'])->order(5);
            
            //restricted menu
            $menu->url(action('RestrictController@index'), __('accounts.restrict_menu'), ['icon' => 'fa fas fa-tachometer-alt', 'active' => request()->segment(10) == 'restrict'])->order(5);
        ...
        //resturant modules
                    ...
                    if( (in_array('booking', $enabled_modules) && (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) 
                     ...
                     ||(in_array('tables', $enabled_modules) && auth()->user()->can('access_tables'))
                     ||(in_array('modifiers', $enabled_modules) && (auth()->user()->can('product.view') || auth()->user()->can('product.create')))
                      ) {
                    ...
                    "المطاعم",
                        ...
                        //Booking menu
                            if (in_array('booking', $enabled_modules) && (auth()->user()->can('crud_all_bookings') || auth()->user()->can('crud_own_bookings'))) {
                                ...
                                $sub->url(
                                    ...
                                    ['icon' => 'fas fa fa-calendar-check', 'active' => request()->segment(100) == 'bookings']
                                    )
                                    ->order(65);                           
                            }
                        ...
                        if (in_array('kitchen', $enabled_modules)) {
                                ...
                                $sub->url(
                                    ...
                                    ['icon' => 'fa fas fa-fire', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'kitchen']
                                )
                                ->order(70)    ;                                
                        ...
                        if(in_array('service_staff', $enabled_modules)) {
                                ...
                                $sub->url(
                                    ...
                                    ['icon' => 'fa fas fa-list-alt', 'active' => request()->segment(1) == 'modules' && request()->segment(2) == 'orders']
                                )
                                ->order(75);                              
...
//                             }
                ...
                function ($sub) {
                    ...
                    $moduleUtil->getModuleData('modifyAdminMenu',$sub);

                }
                ,
                ['icon' => 'fab fa-accusoft', 'id' => 'tour_step9']
            )->order(85);
            
        
        $menu->dropdown('خدمة العملاء',
                function ($sub) {
                        $sub->url(
                            ('https://cloud.weka.sa/clientarea.php'),
                            __('الدعم الفني ومنطقة العميل'),
                            ['icon' => 'fa fas fa-envelope', 'active' => request()->segment(1) == 'notification-templates'])
                            ->order(80); 
                }
                ,
