...
class TransactionUtil extends Util
{
     ...
     * @return array
     */
   public function getReceiptDetails($transaction_id, $location_id, $invoice_layout, $business_details, $location_details, $receipt_printer_type  ,$record=null)
    {
        $il = $invoice_layout;
            $total_line_taxes = 0;
        ...
        if ($transaction_type == 'sell_return') {
            ...
            if ($transaction->payment_status == 'paid' && !empty($il->invoice_heading_paid)) {
                $output['invoice_heading'] .= '<br>' . $il->invoice_heading_paid;
            } elseif (in_array($transaction->payment_status, ['due', 'partial']) && !empty($il->invoice_heading_not_paid)) {
                $output['invoice_heading'] .= '<br>' . $il->invoice_heading_not_paid;
            }
        }
        ...
        $output['tax1'] = ($transaction->tax_amount != 0) ? $this->num_f($transaction->tax_amount, null, $business_details) : 0;
        ...
        if ($transaction_type == 'sell_return') {
            ...
            $output['total1'] = $this->num_f($transaction->final_total,null, $business_details);
        ...
        if(!empty($transaction->commission_agent)){
         $commession = User::find($transaction->commission_agent);
         $output['commession'] = $commession->getUserFullNameAttribute();
        }
        //Check for barcode
    ...
    protected function _receiptDetailsSellLines($lines, $il, $business_details)
    {
        ...
        foreach ($lines as $line) {
            ...
            $line_array = [
                ...
                'unit_price_before_discount_uf' => $this->num_f($line->unit_price_before_discount, false, $business_details),
    ...
    public function _zatca_qr_text($seller, $tax_number, $invoice_date, $invoice_total_amount, $invoice_tax_amount){
        $string = '';
    ...
    public function getInputTax($business_id, $start_date = null, $end_date = null, $location_id = null)
    {
        ...
        if (!empty($start_date) && !empty($end_date)) {
        	...
        	$query1->where('transaction_date', '>=', $start_date)
                    ->where('transaction_date', '<=', $end_date);
            $query2->where('transaction_date', '>=', $start_date)
                    ->where('transaction_date', '<=', $end_date);
    ...
    */
    ...
    public function getExpenseReport(
        ...
        if (!empty($filters['start_date']) && !empty($filters['end_date'])) {
            $query->whereBetween('transaction_date', [$filters['start_date'],
                $filters['end_date']]);
        }
     ...
     * @return void
     */
    public function createOpeningBalanceTransaction($business_id=null, $contact_id=null, $amount=null, $created_by=null, $uf_data = true)
    {
    ...
    */
    ...
    public function getEmailAttachmentForGivenTransaction($business_id, $transaction_id, $is_email_attachment)
    {
        ...
        $mpdf->WriteHTML($body);
    $mpdf->SetDefaultBodyCSS('background', "url('https://test.weka.sa/uploads/invoice_logos/1650100138_background.png')");
